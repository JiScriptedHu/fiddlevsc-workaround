...
export function bisectFiddle(context: vscode.ExtensionContext) {
    console.log('Extension "Command Executor" is now active');

    // Register the command
    let bisect = vscode.commands.registerCommand('commandExecutor.bisectFiddle', async () => {

        // Taking versions
        const startVersion = await vscode.window.showInputBox({
            placeHolder: 'Enter starting electron version',
        ...
        const endVersion = await vscode.window.showInputBox({
            placeHolder: 'Enter ending electron version',
            prompt: 'Please enter the version string'
        });
        ...
        // Execute command
        const command = `fiddle-core bisect ${startVersion} ${endVersion} ${parameterValue}`;
    ...
    context.subscriptions.push(bisect);
}
